...
### 1.4 Agent reliability signals

评估 agent、subagent、skill 或外部 agent 工具是否可靠，不看输出篇幅或自信程度，优先看它是否：

- 主动寻找真实工件、命令输出、官方文档、测试结果或运行时行为。
- 明确任务边界、非目标和 forbidden areas。
- 控制改动面，不顺手重构无关内容。
- 用项目测试、目标系统输出或最终消费者行为验证生效。
- 明确说明未验证部分、剩余风险和下一步。
- 多 agent 场景下，结果能被 reviewer 或汇总节点复核，而不是只依赖 worker 自述。
